  ...
  actions: {
    ...
    Login({ commit }, userInfo) {
      ...
      return new Promise((resolve, reject) => {
        login(userName, password, validCode, deviceId).then(res => {
    ...
    GetInfo({ commit, state }) {
      return new Promise((resolve, reject) => {
        getInfo().then(res => {
          const user = res.data
          const avatar = user.avatar == "" ? require("@/assets/images/profile.gif") : process.env.VUE_APP_BASE_API + "/" + Api.API_SYSTEM + user.avatar;
          console.info(avatar)

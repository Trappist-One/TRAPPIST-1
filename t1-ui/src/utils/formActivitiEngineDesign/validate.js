 ...
 * 5.控件渲染工程图数据不能为空
 **/
import {flatField} from '@/utils/formActivitiEngineDesign'

/** 错误消息 **/
const messageInfo = {
  0: "目前检测到没有组件,请核对后再点击保存!",
  1: "已为你自动定位到字段名为空的组件,请核对后再点击保存!",
  2: "目前动态表单ID为空,请核对后再点击保存!",
...
export default {
  ...
  designValidate: function (data, option) {
    ...
    let jsonMode = data.jsonMode, dynamicFormId = data.dynamicFormId, formConf = jsonMode.formConf
    ...
    /** 判断表单扩展非空 **/
    if (validate.includes(dynamicFormId)) {

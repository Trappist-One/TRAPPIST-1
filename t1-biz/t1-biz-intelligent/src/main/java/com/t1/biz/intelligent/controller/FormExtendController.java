...
import com.t1.biz.intelligent.service.FormExtendService;
...
public class FormExtendController {
    ...
    public R getById(@PathVariable("id") Integer id) {
        return R.success(formExtendService.getById(id));
    }


    @GetMapping("cascade/{id}")
    public R getSelectSingle(@PathVariable("id") Integer id) {
        return R.success(formExtendService.selectSingle(id));
    }
    ...
    public R fetchFormExtend(@PathVariable("code") String code) {
        ...
        return R.success(formExtendService.fetchFormExtend(code));
    }
    ...
    @PostMapping("/save")
    public R save(@LoginUser User user, @RequestBody FormExtend formExtend) {
        if(formExtendService.list(new QueryWrapper<FormExtend>()
            ...
            formExtend.setCreateBy(user.getUserName());
    ...
    @PutMapping("/update")
    public R update(@LoginUser User user, @RequestBody FormExtend formExtend) {
        formExtend.setUpdateBy(user.getUserName());

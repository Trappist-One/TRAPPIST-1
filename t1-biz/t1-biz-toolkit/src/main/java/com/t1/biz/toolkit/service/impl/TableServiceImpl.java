...
import com.t1.biz.toolkit.config.GenConfig;
import com.t1.biz.toolkit.entity.Column;
import com.t1.biz.toolkit.entity.Table;
import com.t1.biz.toolkit.mapper.TableMapper;
import com.t1.biz.toolkit.service.ColumnService;
import com.t1.biz.toolkit.service.TableService;
import com.t1.biz.toolkit.util.BuilderUtil;
import com.t1.biz.toolkit.util.VelocityInitializer;
import com.t1.biz.toolkit.util.VelocityUtil;
import com.t1.common.constant.CommonConstants;
import com.t1.common.constant.GenConstants;
...
import static com.t1.biz.toolkit.util.GenUtil.*;
...
public class TableServiceImpl extends ServiceImpl<TableMapper, Table> implements TableService {
    ...
    @Override
    @Transactional
    public Table getGenTable(String tableName, String tableComment, String username) {
    ...
    public void validateEdit(Table table) {
        if (GenConstants.TPL_TREE.equals(table.getTplCategory())) {
            ...
            if (StrUtil.isEmpty(paramsObj.getStr(GenConstants.TREE_ID))) {
                throw new BusinessException("树编号字段不能为空");
            } else if (StrUtil.isEmpty(paramsObj.getStr(GenConstants.TREE_PARENT_ID))) {
                throw new BusinessException("树父编号字段不能为空");
        ...
        if (table != null) {
            ...
            for (String template : templates) {
                ...
                if (StrUtil.containsAny(template, "vue", "js")) {
                    localFile = table.getGenPath().replace("\\", "/") + "/t1-ui/" + VelocityUtil.getFileName(template, table).replace("vue/", "src/");
                } else if (StrUtil.contains(template, "sql")) {
                    localFile = table.getGenPath().replace("\\", "/") + "/t1-biz/t1-" + table.getModuleName() + "/sql/" + VelocityUtil.getFileName(template, table);
                    sqlPath = localFile;
                } else {
                    localFile = table.getGenPath().replace("\\", "/") + "/t1-biz/t1-" + table.getModuleName() + "/src/" + VelocityUtil.getFileName(template, table);
                }

...
import com.t1.biz.quartz.enums.JobTypeQuartzEnum;
import com.t1.biz.quartz.event.JobLogEvent;
import com.t1.biz.quartz.exception.TaskException;
import com.t1.biz.quartz.service.JobService;
import com.t1.common.utils.SpringContextUtil;
...
public class TaskInvokUtil {
	...
	public void invokMethod(Job job, Trigger trigger) {
		...
		try {
			...
			jobLog.setJobMessage(T1QuartzEnum.JOB_LOG_STATUS_SUCCESS.getDescription());
			jobLog.setJobLogStatus(T1QuartzEnum.JOB_LOG_STATUS_SUCCESS.getType());
			jobLog.setExecuteTime(String.valueOf(endTime - startTime));
			//任务表信息更新
			updateJob.setJobExecuteStatus(T1QuartzEnum.JOB_LOG_STATUS_SUCCESS.getType());
			...
			jobLog.setJobMessage(T1QuartzEnum.JOB_LOG_STATUS_FAIL.getDescription());
			jobLog.setJobLogStatus(T1QuartzEnum.JOB_LOG_STATUS_FAIL.getType());
			jobLog.setExecuteTime(String.valueOf(endTime - startTime));
			jobLog.setExceptionInfo(StrUtil.sub(e.getMessage(), 0, 2000));
			//任务表信息更新
			updateJob.setJobExecuteStatus(T1QuartzEnum.JOB_LOG_STATUS_FAIL.getType());
			updateJob.setStartTime(trigger.getStartTime());
			updateJob.setPreviousTime(trigger.getPreviousFireTime());
			updateJob.setNextTime(trigger.getNextFireTime());
		} finally {

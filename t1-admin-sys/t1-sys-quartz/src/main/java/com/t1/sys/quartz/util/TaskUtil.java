...
import com.t1.sys.quartz.config.T1QuartzFactory;
...
public class TaskUtil {
    ...
    public void addOrUpateJob(Job job, Scheduler scheduler) {
        ...
        try {
            ...
            if (trigger == null) {
                ...
                JobDetail jobDetail = JobBuilder.newJob(T1QuartzFactory.class).withIdentity(jobKey).build();
                //将任务信息添加到任务信息中
                jobDetail.getJobDataMap().put(T1QuartzEnum.SCHEDULE_JOB_KEY.getType(), job);
                //将cron表达式进行转换
                ...
                //将任务信息更新到任务信息中
                trigger.getJobDataMap().put(T1QuartzEnum.SCHEDULE_JOB_KEY.getType(), job);
                //重启
                scheduler.rescheduleJob(triggerKey, trigger);
            }
            // 如任务状态为暂停
            if (job.getJobStatus().equals(T1QuartzEnum.JOB_STATUS_NOT_RUNNING.getType())) {
    ...
    private CronScheduleBuilder handleCronScheduleMisfirePolicy(Job job, CronScheduleBuilder cronScheduleBuilder) {
        if (T1QuartzEnum.MISFIRE_DEFAULT.getType().equals(job.getMisfirePolicy())) {
            return cronScheduleBuilder;
        } else if (T1QuartzEnum.MISFIRE_IGNORE_MISFIRES.getType().equals(job.getMisfirePolicy())) {
            return cronScheduleBuilder.withMisfireHandlingInstructionIgnoreMisfires();
        } else if (T1QuartzEnum.MISFIRE_FIRE_AND_PROCEED.getType().equals(job.getMisfirePolicy())) {
            return cronScheduleBuilder.withMisfireHandlingInstructionFireAndProceed();
        } else if (T1QuartzEnum.MISFIRE_DO_NOTHING.getType().equals(job.getMisfirePolicy())) {

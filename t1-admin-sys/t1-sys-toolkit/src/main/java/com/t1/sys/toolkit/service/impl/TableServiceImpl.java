...
import com.t1.sys.toolkit.config.GenConfig;
import com.t1.sys.toolkit.entity.Column;
import com.t1.sys.toolkit.entity.Table;
import com.t1.sys.toolkit.mapper.TableMapper;
import com.t1.sys.toolkit.service.ColumnService;
import com.t1.sys.toolkit.service.TableService;
import com.t1.sys.toolkit.util.BuilderUtil;
import com.t1.sys.toolkit.util.VelocityInitializer;
import com.t1.sys.toolkit.util.VelocityUtil;
...
import static com.t1.sys.toolkit.util.GenUtil.*;
        ...
        if (table != null) {
            ...
            for (String template : templates) {
                ...
                if (StrUtil.containsAny(template, "vue", "js")) {
                    ...
                    localFile = table.getGenPath().replace("\\", "/") + "/t1-sys/t1-" + table.getModuleName() + "/sql/" + VelocityUtil.getFileName(template, table);
                    sqlPath = localFile;
                } else {
                    localFile = table.getGenPath().replace("\\", "/") + "/t1-sys/t1-" + table.getModuleName() + "/src/" + VelocityUtil.getFileName(template, table);
                }

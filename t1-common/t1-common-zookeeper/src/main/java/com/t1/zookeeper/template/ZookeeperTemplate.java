package com.t1.zookeeper.template;
...
public class ZookeeperTemplate {
    ...
    public List<String> getChildren(String path) {
        ...
        if (!path.startsWith(CommonConstants.PATH_SPLIT)) {
            path = CommonConstants.PATH_SPLIT + path;
        }
    ...
    private String buildPath(String path, String node) {
        ...
        if (CommonConstants.PATH_SPLIT.equals(path)) {
            return path + node;
        } else {
            return path + CommonConstants.PATH_SPLIT + node;
        }
    }
}

package com.t1.feign.loadbalancer;

import com.t1.common.constant.CommonConstants;
import com.t1.feign.chooser.IRuleChooser;
import com.t1.feign.utils.QueryUtils;
...
public class VersionLoadBalancer implements ReactorServiceInstanceLoadBalancer {
    ...
    private String getVersionFromRequestData(RequestData requestData){
        ...
        if(MapUtils.isNotEmpty(queryMap)&& queryMap.containsKey(CommonConstants.T1_VERSION)&& StringUtils.isNotBlank(queryMap.get(CommonConstants.T1_VERSION))){
            return queryMap.get(CommonConstants.T1_VERSION);
        }else if(requestData.getHeaders().containsKey(CommonConstants.T1_VERSION)){
            return requestData.getHeaders().get(CommonConstants.T1_VERSION).get(0);
        }
        return null;
    }
    /**
    ...
    private Response<ServiceInstance> getInstanceResponse(List<ServiceInstance>instances, String version){
        ...
        if(StringUtils.isNotBlank(version)){
            if(CollectionUtils.isNotEmpty(instances)){
                filteredServiceIstanceList = instances.stream()
                        .filter(item->item.getMetadata().containsKey(CommonConstants.METADATA_VERSION)&&
                                version.equals(item.getMetadata().get(CommonConstants.METADATA_VERSION)))
        ...
        if(CollectionUtils.isEmpty(filteredServiceIstanceList)){
            filteredServiceIstanceList = instances.stream()
                    .filter(item->!item.getMetadata().containsKey(CommonConstants.METADATA_VERSION)||
                            StringUtils.isBlank(item.getMetadata().get(CommonConstants.METADATA_VERSION))
                            || "default".equals(item.getMetadata().get(CommonConstants.METADATA_VERSION)))
                    .collect(Collectors.toList());
        }

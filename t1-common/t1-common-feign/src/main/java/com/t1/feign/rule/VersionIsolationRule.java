...
public class VersionIsolationRule extends RoundRobinRule {
    ...
    public Server choose(ILoadBalancer lb, Object key) {
        ...
        if (StrUtil.isNotEmpty(version)) {
            ...
            targetList = upList.stream().filter(
                    server -> version.equals(
                            ((NacosServer) server).getMetadata().get(CommonConstants.METADATA_VERSION)
                    )
        ...
        if (CollUtil.isEmpty(targetList)) {
            ...
            targetList = upList.stream().filter(
                    server -> {
                        String metadataVersion = ((NacosServer) server).getMetadata().get(CommonConstants.METADATA_VERSION);

...
import com.t1.common.annotation.*;
...
public class ExcelUtil<T> {
     ...
     * @return 结果
     */
    public void exportExcel(HttpServletResponse response, HttpServletRequest request, List<T> list, String sheetName) throws Exception{
        this.init(list, sheetName, Excel.Type.EXPORT);
        //response为HttpServletResponse对象
        response.setContentType("application/vnd.ms-excel;charset=utf-8");
        //test.xls是弹出下载对话框的文件名，不能为中文，中文请自行编码
        response.setHeader("Content-Disposition", "attachment;filename=" + FileUtil.setFileDownloadHeader(request, sheetName + new Date().getTime()) + ".xls");
